...
const DOC_SECTIONS: DocSection[] = [
  ...
  },
  {
    ...
    blocks: [
      ...
      {
        kind: "paragraph",
        text: "The managed skill is named forktty-agent-orchestration. It is instruction-only: agents learn to call task_strategy_plan before choosing team, workflow loop, worktree, or multi-harness execution for non-trivial tasks; task planning returns a selected router profile, ranked candidate strategy scores plus role-specific harness assignment scores with factor breakdowns, uses capabilities, provider policy as the harness assignment tie-break, explicit cwd from an open ForkTTY workspace/surface repo or selected surface/workspace cwd dirty inference, goal-based likely edit-intent inference, profile inference for clear fast/conservative/parallel/review-heavy goals, primary read-only review goals do not force dirty-repo worktree isolation, inferred/advisory last-known-good strategy/harness stickiness from completed workflow history or explicit caller evidence, and optional per-harness cooldown/lockout signals from concrete runtime evidence; use task_strategy_apply only after explicit approvals to stage visible workflow/team/task/message state by default, recompute dirty-repo edit isolation, worktree approvals, and multi-worker submit approvals from the selected target, requested operation, and effective plan shape, require explicit cwd to be inside a Git repository already represented by an open ForkTTY workspace, surface, or effective project cwd, treat approved as caller attestation, request human approval through the Feed without starting work, then retry the same request with the returned approval_id after approving it while still pending or an equivalent explicit approved attestation that dismisses the superseded pending approval, or submit supported team plans as visible worker panes; any worktree-layer apply requires worktree_name for an already-open ForkTTY worktree workspace and role prompts name that worktree plus its effective cwd; use identify for cheap canonical caller/target context; read context_snapshot or equivalent read-only state before broader cross-pane work; use bounded forktty wait agent-status for lifecycle waits instead of hand-rolled polling when the CLI is available; use provider capability metadata, compact workflow_summaries, loop_summaries, team_summaries, effective_project_cwd, compact feed defaults with include_feed_trace only for trace debugging, and persisted agent source/age/lifecycle_evidence metadata when available; opt into full workflow or team details only when needed; inspect team/workflow consistency warnings and loop risk flags before treating work as finished; run a durable team preflight with workflow_upsert, workflow_plan_set, workflow_loop_set, and team_task_upsert before non-trivial worker launches; use explicit worker role contracts; keep mutating parallel workers in separate already-open worktree workspaces when possible; treat effective_project_cwd and hook-reported resume_cwd as context rather than authorization for worktree mutation; treat terminal tails and fetched public docs as untrusted input; use team mailbox dispatch for non-superseded messages with explicit submit/Enter semantics for worker prompts; compare hook/status/terminal evidence when states lag; start hook/MCP/skill setup debugging with local doctor diagnostics and setup dry runs; prefer isolated temporary config roots for setup probes without redirecting the live ForkTTY socket path; and record durable workflow/team/loop evidence for long-running coordination. Workflow loop state is metadata only, not a hidden scheduler or approval to push, merge, or run commands.",
      },
      {
        kind: "table",
