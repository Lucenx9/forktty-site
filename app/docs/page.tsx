...
const DOC_SECTIONS: DocSection[] = [
  ...
  },
  {
    ...
    blocks: [
      {
        kind: "paragraph",
        text: "ForkTTY targets Codex, Claude Code, Pi, Antigravity, OpenCode, and shell agents. Managed hooks for Codex, Claude Code, Antigravity, and OpenCode persist session ids, cwd, lifecycle state, last activity, permission prompts, token details where available, and status entries consumed by the Agent HUD; agent rows group lifecycle states, mark the current pane, surface risky permission modes, add source/age metadata and diagnostic lifecycle_evidence for freshness checks against the workspace/provider status row, and provider-scoped HUD metadata is cleared when the last matching session ends, closes, hibernates, or is forgotten. Claude Code team workers launched without explicit permission args use documented permission-mode defaults; Pi review workers default to read-only tools unless explicit Pi tool args are supplied. Managed skills add the policy layer that tells agents when to inspect ForkTTY context, teams, workflows, and terminal state.",
      },
      {
        kind: "list",
        items: [
  ...
  },
  {
    ...
    blocks: [
      ...
      {
        kind: "paragraph",
        text: "High-level CLI wrappers compose existing socket methods for common agent coordination flows. identify is the compact read for canonical workspace/surface/effective_project_cwd plus caller validation; ForkTTY pane environment ids are caller context, so stale caller surface ids fall back to the active workspace focus instead of failing the read. wait agent-status performs bounded read-only lifecycle polling through short context.snapshot reads without terminal text reads. team ask and team review create or update the team, create the task before launching a fresh worker surface, assign it after launch, queue the prompt, and dispatch it with provider-aware terminal submit behavior when submit mode is requested; Claude gets staged text, a short settle, and a separate Enter, while providers that accept it reliably keep text plus carriage-return Enter in one write. The worker is bound to the invoking ForkTTY pane or workspace when available, and MCP team_upsert uses the same pane defaults. Low-level team_worker_launch with worktree_name opens the worker in that already-open worktree workspace and inherits that cwd. Re-run the wrappers to launch a new worker, or use team-message-send plus team-message-dispatch for follow-up prompts to an existing worker. team finish / team_finish verifies open tasks, pending messages, and live-looking worker final states, supports dry-run planning, can close only launch-owned disposable worker panes, normalizes missing worker surfaces as closed, and marks the team done in one finalization step. team_worker_shutdown uses the same provider-aware submit behavior by default, and its close_surface option immediately closes only disposable surfaces created by team_worker_launch; it is cleanup, not proof that the worker processed a graceful shutdown request. Dispatch selects the worker workspace/tab and waits briefly for the embedded terminal surface to become socket-ready before typing. Context snapshots include compact workflow_summaries and team_summaries for leader monitoring; full workflow records, team records, and mailbox message bodies are opt-in with include_workflow_details/include_team_details, feed status/progress trace rows are opt-in with include_feed_trace, effective_project_cwd clarifies the actual project directory, workflow/team consistency warnings surface in risk_flags, and team_worker_health includes final_state for cleanup decisions while using ready-runtime liveness to treat workers as live only when their surface still has a ready terminal runtime.",
      },
      {
        kind: "list",
        items: [
