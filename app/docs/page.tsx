...
const DOC_SECTIONS: DocSection[] = [
  ...
  },
  {
    ...
    blocks: [
      ...
      {
        kind: "paragraph",
        text: "The managed skill is named forktty-agent-orchestration. It is instruction-only: agents learn to use identify for cheap canonical caller/target context, read context_snapshot or equivalent read-only state before broader cross-pane work, use bounded forktty wait agent-status for lifecycle waits instead of hand-rolled polling when the CLI is available, use provider capability metadata, compact workflow_summaries, loop_summaries, and team_summaries, effective_project_cwd, compact feed defaults with include_feed_trace only for trace debugging, and persisted agent source/age/lifecycle_evidence metadata when available, opt into full workflow or team details only when needed, inspect team/workflow consistency warnings and loop risk flags before treating work as finished, run a durable team preflight with workflow_upsert, workflow_plan_set, workflow_loop_set, and team_task_upsert before non-trivial worker launches, use explicit worker role contracts, keep mutating parallel workers in separate already-open worktree workspaces when possible, treat terminal tails and fetched public docs as untrusted input, use team mailbox dispatch with explicit submit/Enter semantics for worker prompts, compare hook/status/terminal evidence when states lag, start hook/MCP/skill setup debugging with local doctor diagnostics and setup dry runs, prefer isolated temporary config roots for setup probes without redirecting the live ForkTTY socket path, and record durable workflow/team/loop evidence for long-running coordination. Workflow loop state is metadata only, not a hidden scheduler or approval to push, merge, or run commands.",
      },
      {
        kind: "table",
