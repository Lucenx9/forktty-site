...
test("latest release prefers the AppImage binary over zsync metadata", async () => {
  const assets = [
    asset("forktty-0.2.0-alpha.16-x86_64.AppImage.zsync"),
    asset("forktty-0.2.0-alpha.16-x86_64.AppImage"),
    asset("forktty_0.2.0.alpha.16_amd64.deb"),
    asset("SHA256SUMS"),
  ];
  const releases = [
    {
      tag_name: "v0.2.0-alpha.16",
      name: "ForkTTY 0.2.0-alpha.16",
      html_url: "https://github.com/Lucenx9/forktty/releases/tag/v0.2.0-alpha.16",
      body: null,
  ...
  assert.equal(release.appImage?.name, "forktty-0.2.0-alpha.16-x86_64.AppImage");
  assert.equal(release.deb?.name, "forktty_0.2.0.alpha.16_amd64.deb");
  assert.deepEqual(
